...
export interface Summary extends Counts { latency: Percentiles; ttft: Percentiles }
...
export interface Filters { channel_id: string; model: string; group: string }
...
export async function getOpsData(range: Range, filters: Filters, signal?: AbortSignal) {
  const response = await api.get<{ success: boolean; data: OpsData }>('/api/ops-monitor', { params: { ...range, ...filters }, signal })
  return requireServerSuccess(response.data).data
}

export async function getOpsEvents(params: Range & Filters & { kind: string; outcome: string; request_id: string; user_id: string; before?: number; before_id?: string }, signal?: AbortSignal) {
  const response = await api.get<{ success: boolean; data: { items: OpsEvent[]; has_more: boolean } }>('/api/ops-monitor/events', { params, signal })
  return requireServerSuccess(response.data).data
}
