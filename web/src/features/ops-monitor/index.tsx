...
import { getOpsData, monitoringRange, type Filters } from './api'
...
export function OpsMonitor() {
  ...
  return <SectionPageLayout>
    ...
    <SectionPageLayout.Actions>
      <Button variant='outline' onClick={() => setPaused(!paused)}>{paused ? t('Resume refresh') : t('Pause refresh')}</Button>
      <Button variant='outline' disabled={!range || query.isFetching} onClick={() => void query.refetch()}><RefreshCw />{t('Refresh')}</Button>
    </SectionPageLayout.Actions>
    <SectionPageLayout.Content><div className='space-y-5'>
      <form className='flex flex-wrap items-center gap-2' onSubmit={(event) => { event.preventDefault(); setFilters(draft) }}>
        <Input className='w-36' aria-label={t('Channel ID')} placeholder={t('Channel ID')} type='number' min={1} value={draft.channel_id} onChange={(event) => setDraft({ ...draft, channel_id: event.target.value })} />
        <Input className='w-48' aria-label={t('Model')} placeholder={t('Model')} maxLength={256} value={draft.model} onChange={(event) => setDraft({ ...draft, model: event.target.value })} />
        <Input className='w-40' aria-label={t('Group')} placeholder={t('Group')} maxLength={128} value={draft.group} onChange={(event) => setDraft({ ...draft, group: event.target.value })} />
        <Button type='submit' variant='outline'>{t('Apply filters')}</Button>
        <NativeSelect aria-label={t('Time range')} value={period} onChange={(event) => setPeriod(event.target.value)}>
          <NativeSelectOption value='1'>{t('Last hour')}</NativeSelectOption>
        ...
        {period === 'custom' && <>
          <Input className='w-auto' type='datetime-local' aria-label={t('Start time')} value={customStart} onChange={(event) => setCustomStart(event.target.value)} />
          <Input className='w-auto' type='datetime-local' aria-label={t('End time')} value={customEnd} onChange={(event) => setCustomEnd(event.target.value)} />
        </>}
        <span className='text-muted-foreground text-xs'>Asia/Shanghai · {t('Refresh every 10 seconds')}</span>
      </form>
      {!range && <Alert variant='destructive'><AlertDescription>{t('Select a valid range within the last 30 days (UTC+8).')}</AlertDescription></Alert>}
      ...
      {range && data && <>
        ...
        {(data.live.write_failed || data.live.dropped > 0 || data.live.pending > 1000) && <Alert variant='destructive'><AlertDescription>{t('Monitoring data may be incomplete. Pending: {{pending}}; dropped since restart: {{dropped}}.', { pending: data.live.pending, dropped: data.live.dropped })}</AlertDescription></Alert>}
