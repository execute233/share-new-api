...
- On native Windows, use PowerShell 7 (`pwsh`) directly; never invoke `cmd.exe` or `cmd /c` for normal command execution.
- Do not nest `pwsh -Command` inside a PowerShell session.
- Prefer direct executable calls and single-quoted paths or patterns containing spaces, Unicode, or special characters.
- Use a temporary `.ps1` script for complex quoting. Invoke `.cmd` and `.bat` programs directly from PowerShell.
