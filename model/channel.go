...
// UpdateWithProxyBinding atomically saves channel edits and an explicitly supplied
// proxy binding, including nil to select direct connections.
func (channel *Channel) UpdateWithProxyBinding() error {
	return channel.update(true)
}

func (channel *Channel) update(proxyBindingProvided bool) error {
	...
	return DB.Transaction(func(tx *gorm.DB) error {
		...
		if proxyBindingProvided {
			if err := tx.Model(channel).Update("proxy_id", channel.ProxyID).Error; err != nil {
				return err
			}
		}
