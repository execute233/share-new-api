...
func UpdateChannel(c *gin.Context) {
	...
	originResolvedProxy, originProxyErr := service.ResolveChannelProxy(originChannel)
	...
	if proxyProvided {
		err = channel.UpdateWithProxyBinding()
	} else {
		err = channel.Update()
	}
	if err != nil {
...
// UnmarshalJSON distinguishes an omitted proxy binding from an explicit null.
func (req *fetchModelsRequest) UnmarshalJSON(data []byte) error {
	type requestAlias fetchModelsRequest
	...
	*req = fetchModelsRequest(decoded)
	_, req.ProxyIDSet = fields["proxy_id"]
	return nil
}
...
func buildAdvancedCustomModelPreviewChannel(req fetchModelsRequest) (*model.Channel, error) {
	...
	if req.ProxyIDSet || req.ProxyID != nil {
