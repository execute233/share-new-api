...
func proxyTaskMedia(c *gin.Context, task *model.Task, descriptor *relaychannel.TaskContentRequest) error {
	...
	if err != nil {
		return &taskMediaProxyError{status: http.StatusServiceUnavailable, code: "artifact_internal_error", message: "Failed to resolve artifact proxy", err: err}
	}
	proxy := resolvedProxy.URL
