...
// Startup migration tests open additional connection pools. Capture each pool
// before another startup replaces the globals, so Windows can remove test files.
...
func initAuditTestLogDB(t *testing.T) {
	t.Helper()
